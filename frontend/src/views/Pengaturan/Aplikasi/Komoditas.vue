<template>
  <div class="aplikasi-komoditas-page">
    <div class="page-header">
      <h1>Komoditas</h1>
      <div class="breadcrumb">
        <RouterLink to="/" title="/">Home</RouterLink> &gt; <RouterLink to="/pengaturan/aplikasi" title="/pengaturan/aplikasi">Pengaturan Aplikasi</RouterLink> &gt; <span>Komoditas</span>
      </div>
    </div>
    ...
    <div class="table-card">
      <div class="toolbar">
        ...
        <button class="btn-outline" @click="triggerFileInput" v-if="hasPermission('Komoditas (Aplikasi)', 'canCreate')">
          Import CSV
        </button>
        <button class="btn-success" @click="exportCSV" v-if="hasPermission('Komoditas (Aplikasi)', 'canRead')">
          Export CSV
        </button>
        <button class="btn-primary" @click="openModal()" v-if="hasPermission('Komoditas (Aplikasi)', 'canCreate')">
          + Tambah Komoditas
        </button>
      </div>
      ...
      <div class="filters">
        ...
        <div class="filter-right">
          <input type="text" v-model="searchQuery" class="form-control" placeholder="Masukkan Nama Komoditas Lalu Tekan Enter" @keyup.enter="currentPage = 1" />
        </div>
      </div>
        ...
        <tbody>
          ...
          <tr v-for="item in paginatedData" :key="item.id">
            ...
            <td style="text-align: right;">
              <button class="btn-icon text-primary" @click="openModal(item)" title="Edit" v-if="hasPermission('Komoditas (Aplikasi)', 'canUpdate')">
                ✎
              </button>
              <button class="btn-icon text-danger" @click="deleteItem(item.id)" title="Hapus" v-if="hasPermission('Komoditas (Aplikasi)', 'canDelete')">
                🗑
              </button>
            </td>
          </tr>
    ...
    <div class="modal-backdrop" v-if="showModal">
      <div class="modal-content">
        <div class="modal-header">
          <h3>{{ form.id ? 'Edit' : 'Tambah' }} Komoditas</h3>
          <button class="close-btn" @click="closeModal">&times;</button>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>Nama Komoditas</label>
            <input type="text" v-model="form.nama" class="form-control" placeholder="Contoh: SEMBAKO / ELEKTRONIK" style="width: 100%; box-sizing: border-box;" />
          </div>
        ...
        <div class="modal-footer">
          ...
          <button class="btn-primary" @click="saveData" v-if="hasPermission('Komoditas (Aplikasi)', 'canCreate')">{{ form.id ? 'Simpan Data' : 'Tambah Data' }}</button>
        </div>
      </div>
    </div>
  </div>
...
const exportCSV = () => {
  ...
  const headers = ['ID', 'Nama Komoditas', 'Default'];
  ...
  link.setAttribute('download', 'komoditas_export.csv');
...
const handleImportFile = (event) => {
  ...
  reader.onload = async (e) => {
    ...
    // Skip header assuming format: Nama Komoditas, Default
...
<style scoped src="../../../assets/css/aplikasi-komoditas.css"></style>

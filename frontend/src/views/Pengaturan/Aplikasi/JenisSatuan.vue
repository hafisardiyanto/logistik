...
const fetchData = async () => {
  try {
    const res = await axios.get('http://localhost:3000/api/jenis-satuan');
    if (res.data.success) {
...
const saveData = async () => {
  ...
  try {
    let res;
    if (form.value.id) {
      res = await axios.put(`http://localhost:3000/api/jenis-satuan/${form.value.id}`, form.value);
    } else {
      res = await axios.post('http://localhost:3000/api/jenis-satuan', form.value);
    }
    if (res.data.success) {
...
const deleteItem = async (id) => {
  ...
  try {
    const res = await axios.delete(`http://localhost:3000/api/jenis-satuan/${id}`);
    if (res.data.success) {
      fetchData();
    }
  } catch (error) {
...
const handleImportFile = (event) => {
  ...
  reader.onload = async (e) => {
    ...
    if (importData.length > 0) {
      try {
        const res = await axios.post('http://localhost:3000/api/jenis-satuan/bulk', importData);
        if (res.data.success) {
